...
do
  ...
  charles -headless -config  ${charles_proxy_configuration_file_path}  &
  for pais in "${paises[@]}"
  do
    export PAIS="$pais"
    ...
    robot  --outputdir    ../Resultados/     --variable pais:${pais}  --variable pais_url:${PAIS_URL}  ${TESTNAME}.robot
